...
exports.createEvent = async (req, res) => {
  try {
    ...
    functions.sendNotificationToAll({
      ...
      contents: {
        en: ` ${user.firstName} ${user.lastName} will be live on ${eventDate}, do you want to notified when they are live?.`,
      },
    });
...
const startRecording = async (resourceid, uid, channelname, token) => {
  ...
  let plainCredentials =
    ...
    settingsresponse["AGORA_CUSTOMER_SECRET"];
  let AppId = settingsresponse["agoraAppID"];
  let vendor = settingsresponse["AWSVENDOR"];
  let region = settingsresponse["AWSREGION"];
  let bucket = settingsresponse["AWSBUCKET"];
  let bucketAccessKey = settingsresponse["AWSACCESSKEY"];
  let bucketSecretKey = settingsresponse["AWSSECRETKEY"];
  ...
  let url = `https://api.agora.io/v1/apps/${AppId}/cloud_recording/resourceid/${resourceid}/mode/mix/start`;
  console.log(url);
  ...
  return axios
    ...
    .then(async (response) => {
      console.log("startRecording", response.data);
